...
plan; `kernel-tv-goals.md` gives the *why* + success criteria, `kernel-tv-design.md`
...
Move today's Triton-coupled implementation onto the kernel-tv architecture. **Two
sides:**
- **SMT side** — extract `kernel-tv` (core) out of `semantics/`: MLIR-free,
...
- **Triton side** — turn the current op handlers + `State`/`Env` into a thin
  **Triton adapter** that walks TTIR and calls the kernel-tv builder.
- *Done when:* the eval suite (`run_eval.py all`) is green through the adapter,
  and `kernel-tv` has zero MLIR includes/symbols.
- *Detailed split plan:* `doc/m0-plan.md`.

### M1 — extend kernel-tv to model (almost) all of Triton TTIR
Grow the kernel-tv semantic model, driven by TTIR's needs, until it cleanly models
...
### M2 — model kernel-gpu-tv (TTGIR / GPU layer)
...
framework independence). Its own adapter drives the same kernel-tv / kernel-gpu-tv.
...
An **independent, dynamic** line that does **not** use kernel-tv. It is the M
...
on vs off) and feed them to **M's validator (kernel-tv)** — does it also catch the
...
## Validation experiments (V) — static equivalence via kernel-tv

Exercises **kernel-tv's own power** and is the real use of the tool to hunt
