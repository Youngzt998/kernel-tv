...
namespace `kernel_tv`) and the **builders** (`builder/`, namespace
...
builder/  (namespace Semantics)   walks mlir::Operation, reads MLIR types/attrs, calls core
   ...
   │  depends on
   ▼
semantics/  (namespace kernel_tv)  pure Z3 op semantics — NO MLIR
...
Rule: **core includes no MLIR/Triton**. Verify: `grep -rE 'mlir::|triton::|#include "mlir' semantics/` is empty; `libkernel-tv.a` links z3 only.
...
|---|---|---|
| **MLIR-free core** | `semantics/` | `grep -rE 'mlir::\|triton::\|#include "mlir' semantics/` empty; `ar t libkernel-tv.a` = core .o only |
...
See also: `doc/m0-plan.md` (structure/CMake), `paper/noticable.md` (store-blowup fix rationale), `CLAUDE.md` (current state).
